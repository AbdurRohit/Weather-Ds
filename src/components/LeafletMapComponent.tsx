...
function PolygonManager({ selectedTime, timeRange }: PolygonManagerProps) {
  ...
  const handleDataSourceSelect = async (dataSource: string, name: string) => {
    console.log('Creating polygon with points:', tempPolygonPoints, 'at time:', selectedTime);
    ...
    // Add polygon and immediately update its weather data
    await polygonStore.addPolygon(newPolygon)
    if (selectedTime) {
      await polygonStore.updatePolygonWeather(newPolygon.id, selectedTime, timeRange)
    }
...
export default function LeafletMapComponent({ selectedTime, timeRange }: LeafletMapComponentProps) {
  ...
  return (
    <div className="w-full h-full min-h-[500px] min-w-[350px]">
      ...
      >
        <PolygonManager selectedTime={selectedTime} timeRange={timeRange} />
        <MapControls />
        <TileLayer
